...
pub fn run(
    ...
    info!(
        ...
        citygml_model.wall_surface().len()
    );
    ...
    info!(
        ...
        transformed_citygml_model.wall_surface().len()
    );

    let triangle_mesh = ecitygml_converter::citymodel_to_mesh(transformed_citygml_model).unwrap();
